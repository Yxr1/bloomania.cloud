package com.bloomania.cloud.views.edit_posts;

import com.bloomania.cloud.data.SampleBook;
import com.bloomania.cloud.services.SampleBookService;
...
@PageTitle("Edit_posts")
@Route("edit_posts/:sampleBookID?/:action?(edit)")
@Menu(order = 2, icon = LineAwesomeIconUrl.COLUMNS_SOLID)
@RolesAllowed("ADMIN")
public class Edit_postsView extends Div implements BeforeEnterObserver {

    private final String SAMPLEBOOK_ID = "sampleBookID";
    private final String SAMPLEBOOK_EDIT_ROUTE_TEMPLATE = "edit_posts/%s/edit";
    ...
    public Edit_postsView(SampleBookService sampleBookService) {
        this.sampleBookService = sampleBookService;
        addClassNames("editposts-view");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(Edit_postsView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleBookId.isPresent()) {
            ...
            if (sampleBookFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(Edit_postsView.class);
            }
        }
    }
