package com.bloomania.cloud.views.editprojects;
...
@Route("edit-projects/:sampleBookID?/:action?(edit)")
@Menu(order = 5, icon = LineAwesomeIconUrl.COLUMNS_SOLID)
@RolesAllowed("ADMIN")
public class EditProjectsView extends Div implements BeforeEnterObserver {

    private final String SAMPLEBOOK_ID = "sampleBookID";
    private final String SAMPLEBOOK_EDIT_ROUTE_TEMPLATE = "edit-projects/%s/edit";
    ...
    public EditProjectsView(SampleBookService sampleBookService) {
        this.sampleBookService = sampleBookService;
        addClassNames("edit-projects-view");
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(EditProjectsView.class);
            }
        });
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (sampleBookId.isPresent()) {
            ...
            if (sampleBookFromBackend.isPresent()) {
                ...
                // refresh grid
                refreshGrid();
                event.forwardTo(EditProjectsView.class);
            }
        }
    }
